...
// Function to detect repetitive patterns and gibberish (adadadadadadaa, sasdaasdasdadasasasd, Kukukukuku)
const hasRepetitivePattern = (str) => {
    ...
    // This catches: "kukuku", "lala", "nana", etc.
...
// Enhanced name validation - allow single characters and better space handling
const isValidName = (str) => {
    ...
    // Examples: "John", "J.", "O'Brien", "Mary Ann", "Jean-Paul"
...
const rules = {
    ...
    username: { 
        ...
        msg: 'Letters/numbers/underscore, min 3. No dummy/repeated patterns',
        customValidator: (val) => {
    ...
    postal: { 
        ...
        msg: 'Valid postal code required',
        customValidator: (val) => {
            if (hasRepetitivePattern(val)) return false;
            return true;
        }
...
const filters = {
    name: v => {
        ...
        if (/^\.?[A-Za-z]\.?$/.test(v)) {
            // Just capitalize the letter, don't change the dot position
            v = v.replace(/[a-z]/g, c => c.toUpperCase());
        } else {
            // Capitalize first letter of each word (after space, dot, hyphen, apostrophe)
            v = v.replace(/^[a-z]/, c => c.toUpperCase()); // First character
            v = v.replace(/\s[a-z]/g, c => c.toUpperCase()); // After space
            v = v.replace(/\.[a-z]/g, c => c.toUpperCase()); // After dot
            v = v.replace(/-[a-z]/g, c => c.toUpperCase()); // After hyphen
            v = v.replace(/'[a-z]/g, c => c.toUpperCase()); // After apostrophe
        }
        return v;
    },
    ...
    postal: v => v.replace(/[^A-Za-z0-9]/g, '')
};
...
// Add DOB input validation to restrict year format
document.getElementById('dob').addEventListener('change', function() {
    ...
    if (dateValue) {
        ...
        if (year < 1900 || year > currentYear) {
            this.value = '';
            document.getElementById('dobError').textContent = 'Year must be between 1900 and ' + currentYear;
...
document.getElementById('dob').addEventListener('keydown', function(e) {
    // Allow only numbers and standard date input keys
    if (!/[\d\-\/:]/i.test(e.key) && !['Backspace', 'Delete', 'Tab', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
        e.preventDefault();
    }
});
...
const validate = field => {
    ...
    if (field.id === 'confirmPassword') {
        ...
        if (!val) { 
            ...
            const age = Math.floor((Date.now() - new Date(val)) / 31557600000);
            valid = age >= 13;
