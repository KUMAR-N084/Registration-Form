...
const dummyDataBlacklist = {
    ...
    names: [
        'test', 'admin', 'demo', 'sample', 'user', 'john', 'jane', 'dummy'
    ],
    phones: [
...
// Function to detect repetitive patterns and gibberish (adadadadadadaa, sasdaasdasdadasasasd)
const hasRepetitivePattern = (str) => {
    // Check for patterns like "adadada" or "asdasd"
    ...
    // Check for consonant-heavy gibberish (random keyboard mashing)
...
const isValidName = (str) => {
    ...
    // Allow: single letter, letters with spaces/dots, apostrophes for names
    // Examples: "A", "John", "J.", "O'Brien", "Mary Ann"
...
document.getElementById('postalCode').addEventListener('blur', function() {
    const loc = postalCodes[this.value.trim().toUpperCase()];
    if (loc) {
        ['country', 'state', 'city'].reduce((p, k) => p.then(() => new Promise(r => {
            setTimeout(() => {
...
const validate = field => {
    ...
    if (field.id === 'confirmPassword') {
        ...
        if (!val) { msg = 'Date of birth required'; }
        else {
            ...
            consentSection.classList.toggle('hidden', age >= 18 || age < 13);
...
document.querySelectorAll('[data-rule]').forEach(f => {
    ...
    if (filters[rule]) {
        f.addEventListener('input', e => {
            ...
            if (rule === 'name' && e.target.value.length === 1) e.target.value = e.target.value.toUpperCase();
            validate(f);
        });
    } else {
